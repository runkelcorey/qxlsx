...
internal <- readWorkbook("internal-cfrt.xlsx", sheet = "Data", detectDates = T) %>% #read database
  ...
  add_column(ran = runif(nrow(.))) %>%
  mutate(id = paste(countrycode(country, origin="country.name", destination = "iso3c"), substr(`Institutional.Role(s)`, 1, 1), `Adoption/Proposal.Date.(MM-DD-YYYY)`, substr(ran, 3, 9), sep = "_")) #construct unique persistent id: country_role_date_number
